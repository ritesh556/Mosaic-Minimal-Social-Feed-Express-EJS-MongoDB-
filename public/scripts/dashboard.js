...
(function () {
  ...
  const __me = boot.user || null;
  const __followersPreview = boot.followersPreview || [];
  const __followingPreview = boot.followingPreview || [];
  ...
  function buildMutuals() {
    ...
    (__followingPreview || []).forEach((u) => {
      ...
      if (byId.has(id)) byId.get(id).g = true;
      else byId.set(id, { ...u, f: false, g: true });
    });
  ...
  async function loadChats() {
    ...
    try {
      ...
      const chats = await r.json(); // expect [{other, lastMsg, unreadCount}, ...]
      renderChatList(chats);
      const total = (chats || []).reduce((n, c) => n + (c.unreadCount || 0), 0);
  ...
  function renderMessages(msgs) {
    const meId = String(__me && __me._id || "");
    const atBottom = (threadBodyEl.scrollHeight - threadBodyEl.scrollTop - threadBodyEl.clientHeight) < 100;
    threadBodyEl.innerHTML =
      (msgs || [])
        .map((m) => {
